...
def warp_image(img, tform, order=None, mode='constant', cval=0.0, preserve_range=False):
    tmp2 = np.copy(tform)
...
def findTransformECC(templateImage, inputImage, warpMatrix=None, motionType='MOTION_AFFINE', numberOfIterations=200, termination_eps=-1, inputMask=None, gaussFiltSize=5):
    ...
    for i in range(numberOfIterations):
        ...
        if den <= 0.0:
            rho = -1
            raise Exception("The algorithm stopped before its convergence. The correlation is going to be minimized. Images may be uncorrelated or non-overlapped")
